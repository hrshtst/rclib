...
def plot_results(df: pd.DataFrame, output_dir: Path, plot_suffix: str) -> None:
    ...
    try:
        ...
        plt.yscale("log")
        plt.xscale("log")
        plt.title("Training Time: rclib vs reservoirpy", fontsize=title_fs)
        ...
        fit_plot_path = output_dir / f"comparison_fit_time{plot_suffix}"
        plt.tight_layout()
        ...
        plt.yscale("log")
        plt.xscale("log")
        plt.title("Prediction Time: rclib vs reservoirpy", fontsize=title_fs)
        plt.ylabel("Time (s) - Log Scale", fontsize=label_fs)
        plt.xlabel("Number of Neurons - Log Scale", fontsize=label_fs)
        plt.xticks(fontsize=tick_fs)
        plt.yticks(fontsize=tick_fs)
        plt.legend(fontsize=legend_fs)
        plt.grid(visible=True, which="both", ls="-", alpha=0.5)
        pred_plot_path = output_dir / f"comparison_pred_time{plot_suffix}"
        plt.tight_layout()
        plt.savefig(pred_plot_path)
        print(f"Plots saved to {output_dir}/comparison_*")
    except ImportError:
...
def main() -> None:
    """Run comparison."""
    parser = argparse.ArgumentParser(description="Benchmark comparing rclib and reservoirpy.")
